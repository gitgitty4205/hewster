...
- Today's timeline card format is locked to match History Full Timeline: compact `p-2.5` card, `1.35rem` marker column, size-5 icon marker aligned with heading, `mt-1` details, and the compact `px-2 py-0.5 text-[11px]` time badge. Run `npm run check:timeline-format` after timeline/card styling changes.
